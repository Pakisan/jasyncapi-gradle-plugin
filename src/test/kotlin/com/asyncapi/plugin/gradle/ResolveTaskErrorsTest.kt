...
open class ResolveTaskErrorsTest {
    ...
    fun checkEmptyPackageNamesAndClassNamesExceptionTest() {
        ...
        Assertions.assertTrue(errorWriter.buffer.contains(
            """
                * What went wrong:
                Execution failed for task ':resolve'.
                > Can't generate schema: [required parameters]: classNames or packageNames are required
            """.trimIndent(),
            true)
        )
    }
    ...
    fun wrongSchemaFileFormatException() {
        ...
        Assertions.assertTrue(errorWriter.buffer.contains(
            """
                * What went wrong:
                Execution failed for task ':resolve'.
                > Can't generate schema: [generation strategy]: text not recognized
            """.trimIndent(),
            true)
        )
    }
